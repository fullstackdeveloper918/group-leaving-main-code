...
const ContributorsModal: React.FC<ContributeModelProps> = ({ isOpen, onClose, groupId , reloadlyId }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/order/transaction-list?cartId=${groupId}&reloadlyId=${reloadlyId}`)
      .then(res => res.json())
      .then(data => {
          ...
          <div className="max-h-72 overflow-y-auto mb-4">
            {contributors.map((c, idx) => (
              <div key={idx} className="py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <div>
                  <div className="font-semibold text-gray-800">{c.senderName || "Anonymous"}</div>
                ...
                <div className="text-right">
                  <div className="text-blue-700 font-bold text-lg">₹{c.amount}</div>
