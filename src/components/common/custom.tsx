...
// Initial elements for Slide 2 (index 1): 3 images, 2 GIFs, 3 texts
...
const Custom: React.FC = () => {
  ...
  const fetchGifs = async (term: string, type: 'GIF' | 'Sticker' = 'GIF') => {
    try {
      ...
      const gifUrls = response.data.results.map((result: any) =>
        type === "Sticker"
          ? result.media_formats?.tinygif_transparent?.url || result.media_formats?.gif?.url
  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (searchTerm) fetchGifs(searchTerm, type);
  };
  ...
  // Ensure elements are unique to prevent duplicates
                      ...
                      uniqueElements
                        ...
                        .map((el: any, i: any) => (
                          <DraggableElement
                            ...
                            content={el.content}
                            type={el.type}
                            index={{ original: elements.findIndex((e) => e === el), activeSlide: activeSlideIndex }}
                            setElements={setElements}
                            elements={elements}
                            initialX={el.x || 0}
                            initialY={el.y || 0}
